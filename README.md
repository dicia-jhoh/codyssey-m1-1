...
![Reproducible](https://img.shields.io/badge/난수-없음%20(결정적)-brightgreen)
![Deploy](https://img.shields.io/badge/OCI%20배포-불필요-lightgrey)
...
- [이 미션의 위치](#이-미션의-위치)
- [1. 분석 질문](#1-분석-질문)
- [2. 데이터 확인과 전처리](#2-데이터-확인과-전처리)
- [3. 적용한 시계열 기법 (4가지)](#3-적용한-시계열-기법-4가지)
- [4. 시각화와 인사이트](#4-시각화와-인사이트)
- [5. 보너스 (A) — 시계열 분해](#5-보너스-a--시계열-분해)
- [5.5 보너스 (C) — 탐색 대시보드](#55-보너스-c--기간기법을-바꿔-보는-탐색-대시보드)
- [6. 보너스 (B) — 베이스라인 예측](#6-보너스-b--베이스라인-예측)
- [7. 결론](#7-결론)
- [8. AI 사용 로그 (의무)](#8-ai-사용-로그-의무)
- [9. 실행 방법](#9-실행-방법)
- [10. 용어 사전](#10-용어-사전)
- [11. 준비물 (전제 지식 0)](#11-준비물-전제-지식-0)
- [12. 따라 하기](#12-따라-하기)
...
```mermaid
flowchart LR
    R["원계열<br/>144개월"] --> A["① 12개월 중심 이동평균<br/>추세"]
    R --> B["② 변화율<br/>전월비 · 전년동월비"]
    R --> C["③ 월별 계절 지수<br/>연평균 대비 배수"]
    R --> D["④ 연도별 구간 통계<br/>변동계수(CV)"]
    A --> S["종합 인사이트"]
    B --> S
    C --> S
    D --> S
...
> **제출 방식(택1) 충족**: 미션은 대시보드 제출을 (1) 배포 URL · (2) 실행 화면 녹화 ·
> (3) 스크린샷 세트 + 시나리오 설명 **셋 중 하나**로 정하고 있고, 이 저장소는
> **(3) 스크린샷 세트 + 필터 변경 시나리오**로 요건을 충족했습니다(아래 3장 · `images/`).
> 이 대시보드는 서버 없는 단일 HTML 이라 파일을 열면 그대로 동작합니다 — 별도 호스팅
> 없이 누구나 같은 화면을 재현할 수 있고, 정지 화면이 필터 변경 전후를 나란히 비교하기에
> 가장 적합합니다.
